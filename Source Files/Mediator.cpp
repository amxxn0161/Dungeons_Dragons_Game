...
void Mediator::UpgradeAttackPoints(){ //Function for upgrading attack points
    Monk::instance().setAttackPoints(Monk::instance().getAttackPoints() + 2);
}

void Mediator::UpgradeHealthPoints(){ //Function for upgrading health
    Monk::instance().setHealthPoints(Monk::instance().getHealthPoints() + 5);
}
